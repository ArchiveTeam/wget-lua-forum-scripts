--[[
Download script for phpBB3 forums.
2013.03.27
...
the index page without and with index.php. For example:
 http://www.phpbb.com/community/feed.php
...
wget.callbacks.get_urls = function(file, url, is_css, iri)
  ...
  if base then
    ...
    -- pages
    for start in string.gmatch(html, "viewforum%.php%?f="..forum_id.."&amp;start=(%d+)") do
      table.insert(urls, { url=(base.."/viewforum.php?f="..forum_id.."&start="..start), link_expect_html=1 })
    end

    -- topics
    for topic_id in string.gmatch(html, "viewtopic%.php%?f="..forum_id.."&amp;t=(%d+)") do
      table.insert(urls, { url=(base.."/viewtopic.php?f="..forum_id.."&t="..topic_id), link_expect_html=1 })
      -- first page
      table.insert(urls, { url=(base.."/viewtopic.php?f="..forum_id.."&t="..topic_id.."&start=0"), link_expect_html=1 })
    end

    return urls
  end

  base, forum_id, topic_id = string.match(url, "(http://.+)/viewtopic%.php%?f=(%d+)&t=(%d+)")
  if base then
    -- topic
    html = read_file(file)

    -- pages
    for start in string.gmatch(html, "viewtopic%.php%?f="..forum_id.."t="..topic_id.."&amp;start=(%d+)") do
      table.insert(urls, { url=(base.."/viewtopic.php?f="..forum_id.."t="..topic_id.."&start="..start), link_expect_html=1 })
    end

    -- profiles
    for u in string.gmatch(html, "memberlist%.php%?mode=viewprofile&amp;u=(%d+)") do
      table.insert(urls, { url=(base.."/memberlist.php?mode=viewprofile&u="..u), link_expect_html=1 })
    end
